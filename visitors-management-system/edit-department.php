...
if(isset($_REQUEST['sv-dpt']))
{
   
	$department_name = $_POST['deptname'];
  $status = $_POST['status'];

  $update_department = mysqli_query($conn,"update tbl_department set department='$department_name', status='$status' where id='$id'");
...
<?php include('include/side-bar.php'); ?>
  ...
  <div class="card mb-3">
      ...
      <label class="col-lg-4 col-form-label" for="department">Department Name <span class="text-danger">*</span></label>
       <div class="col-lg-6">
      <input type="text" name="deptname" id="deptname" class="form-control" placeholder="Enter Department Name" required value="<?php echo $row['department']; ?>">
       </div>
      </div> 
      <div class="form-group row">
